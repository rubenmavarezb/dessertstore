//Variables
let body; //Variable that will call the body tag
let nav; //Variable that will call the nav tag
let overlay; //Variable that will call the .overlay class
let desserts; //Variable that will be converting into a object
let shoppingBasket;
let modal;
let closeModal;
let closeCartModal;
let cartModal;
let buyBtn;
let cart; //Variable that will call the ID #ad-to-basket
let formSearch; //Variable that will call the form tag
let searchBox; //Variable that will call the ID #search-box
let searchButton; //Variable that will call the ID #search-button
let keyframes;
let options;
...
const getSearchValue = () => {
    ...
    let searchResult = desserts.search(searchBoxValue);
...
const renderResume = () => {
    ...
    let addressOne = $('input[name="address1"]');
    let addressTwo = $('input[name="address2"]');
    let countrySelect = $('select[name="country"] option:checked');
    let city = $('input[name="city"]');
    let shipZipCode = $('input[name="zipcode"]')
    let phoneNumb = $('input[name="phonenumber"]')
    let textArea =$('textarea[name="textarea"]')
    let cardNumber = $('input[name="cardnumber"]')
    let month= $('select[name="selectmonth"] option:checked')
    let year= $('select[name="selectyear"] option:checked')
    let payZipCode = $('input[name="zipcode2"]')
...
window.onload = () => {
    ...
    $.ajax({
        method: "GET",
        url: "http://127.0.0.1:5503/coderhouse-proyect/assets/scripts/data.json"
    }).done((data) => {
        desserts = new Products();
        desserts.init(data);
    ...
    //Value assignation to the variables declared before. sE is an abrevation of 'selectElement', is a selector created by me. You can find the structure of it in the file products.js
    body = sE('body');
    nav = sE('nav');
    overlay = sE('.overlay');
    cart = sE('#cart-length');
    formSearch = sE('#search-container');
    searchBox = sE("#search-box");
    searchButton = sE('#search-button');
    modal = sE('#modal-section');
    closeModal = sE('#view-more-close');
    closeCartModal = sE('#close-cart-modal');
    cartModal = sE('#cart-modal');
    buyBtn = sE('#buy-cart-btn')
